...
const RestrictedWordModal = ({ open, onClose }) => {
  ...
  const { uploadRestrictedWordCsvService, createRestrictedWordService } =
    useRestrictedWord();

  // FIX: use a ref + programmatic click for reliable “Choose file”
  ...
  const handleUpload = async () => {
    ...
    try {
      setUploading(true);
      setMessage(null);
      await uploadRestrictedWordCsvService(file);
  ...
  const handleManualSubmit = async () => {
    const value = manualRestrictedWord.trim();
    if (!value) {
      setMessage({ type: "error", text: "Please enter an Restricted Word." });
      return;
    }
    try {
      setCreating(true);
      setMessage(null);
      await createRestrictedWordService(value);
      setMessage({ type: "success", text: "Restricted Word added." });
      setManualRestrictedWord("");
      onClose?.();
    } catch (e) {
      setMessage({
        type: "error",
        text: e?.data?.message || e?.message || "Failed to add Restricted Word",
      });
    } finally {
      setCreating(false);
    }
    ...
    >
      ...
      {isBusy && (
        <ThemeLoader
          type="overlay"
          message={
            uploading ? "Uploading CSV..." : "Creating Restricted Word..."
          }
        />
      )}
      ...
      >
        ...
        >
          <FaCube size={18} style={{ color: "#fff" }} /> ADD Restricted Words
        </DialogTitle>
      ...
      <DialogContent dividers sx={{ backgroundColor: "var(--color-bg)", p: 0 }}>
        ...
        <Stack alignItems="center" sx={{ px: 2, pb: 1 }}>
          <Paper
            ...
            role="tablist"
            aria-label="Add Restricted Word input mode"
          >
            <ToggleButtonGroup
              value={isUploadMode}
              exclusive
        ...
        <Box sx={{ p: 2, pt: 2.5 }}>
          {isUploadMode ? (
            ...
            >
              ...
              >
                Input Single Restricted Word
              </Typography>

              <ThemeTextField
                placeholder="e.g., xxx"
                variant="outlined"
                value={manualRestrictedWord}
                onChange={(e) => setManualRestrictedWord(e.target.value)}
                fullWidth
                className="mb-2"
                inputProps={{ "aria-label": "RestrictedWord" }}
              />
              ...
              <Box display="flex" justifyContent="flex-end" sx={{ mt: 1 }}>
                <ThemeButton
                  ...
                  loading={creating}
                  disabled={!manualRestrictedWord.trim()}
                  tone="success"
                >
                  <FaPlusSquare size={15} className="mr-2" /> Add RestrictedWord
                </ThemeButton>
              </Box>
            </Paper>
          )}
